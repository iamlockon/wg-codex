...
impl HealthReporterConfig {
    fn from_env() -> Option<Self> {
        if std::env::var("CORE_ENTRY_HEALTH_URL").is_ok()
            || std::env::var("CORE_ENTRY_NODE_UPSERT_URL").is_ok()
        {
            warn!(
                "CORE_ENTRY_HEALTH_URL/CORE_ENTRY_NODE_UPSERT_URL are deprecated; entry now pulls node status from the blob catalog"
            );
        }
...
async fn health_report_loop(_service: CoreService, cfg: HealthReporterConfig) {
    warn!(
        node_id = %cfg.node_id,
        "core health reporter is disabled; entry now refreshes node health by polling core targets from the node catalog"
    );
}
